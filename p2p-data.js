...
async function fetchOkxSide(side) {
  ...
  const json = await res.json();
  const list = json?.data?.[0]?.sell || json?.data?.[0]?.buy || json?.data || [];
...
exports.handler = async function () {
  ...
  const pack = (buy, sell, name) => {
    ...
    return { available: true, buy: buy.data, sell: sell.data };
  };
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      fiat: FIAT,
      exchanges: {
        binance: pack(binanceBuy, binanceSell, "binance"),
        bybit: pack(bybitBuy, bybitSell, "bybit"),
        okx: pack(okxBuy, okxSell, "okx"),
      },
    }),
  };
};
